...
function bodySize(c) {
  return Math.abs(p(c.close) - p(c.open));
}

function rangeSize(c) {
  return p(c.high) - p(c.low);
}

export function checkTriggers(candles) {

  if (candles.length < 6) return { triggered: false };
  ...
  const minSweep = 2 * PIP;

  const body = bodySize(c3);
  const range = rangeSize(c3);

  const bodyRatio = body / range;

  const priorSweepHigh = p(c2.high) > p(c1.high);
  const priorSweepLow = p(c2.low) < p(c1.low);
  ...
  if (
    ...
    const confirmBody = bodySize(c3);

    if (
      close < prevLow &&
      wickConfirm <= 0.8 * PIP &&
      confirmBody > wickConfirm
    ) {
